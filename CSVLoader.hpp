...
struct DataPoint {
    std::vector<float> features; // 24 sensor readings
    int label; // 0 (No Collision) or 1 (Potential Collision)
};
...
std::vector<DataPoint> loadDataset(const std::string& filename) {
    ...
    while (std::getline(file, line)) {
        ...
        for (int i = 0; i < 24; ++i) {
            ...
            dp.features.push_back(std::stof(value));
        }
        // Read label
        std::getline(ss, value);
        // Map labels to binary: "Move-Forward" = 0, others = 1
        dp.label = (value == "Move-Forward") ? 0 : 1;
...
// Miniature Mobile Robot Dataset
    // horseman73r@gmail.com
    // THcUj*nJt2H_XeF
...
struct DataPoint {
    std::vector<float> features; // e.g., [distance, signal_strength, angle]
    int label; // 0 or 1
};
...
bool saveDatasetWithIDs(const std::string& filename, const std::vector<DataPoint>& data) {
    ...
    // Write header
    file << "ID";
    for (int i = 1; i <= 24; ++i) file << ",Sensor" << i;
    file << ",Label\n";
    // Write data
    for (const auto& dp : data) {
        file << dp.id;
        for (double f : dp.features) file << "," << std::fixed << std::setprecision(3) << f;
        file << "," << dp.label << "\n";
    }
    file.close();
    return true;
